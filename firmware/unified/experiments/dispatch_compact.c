...
static void close_current(wd_dispatch *d, uint32_t now) {
    d->state = WD_CLOSED;
    d->offered = false;
    /* rx/reply are unreachable while closed. A later accepted request runs
     * ww_rx_begin before reading rx.body; ready overwrites every reply byte. */

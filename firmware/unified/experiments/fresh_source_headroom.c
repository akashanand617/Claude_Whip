...
ws_result ws_observe(ws_source *s, const ws_receipt *r,
     ...
     */
    /* UNADOPTED arithmetic experiment. Earlier checks prove count<=32 and
     * span<250; a valid profile supplies positive u16 minimum_period_ms.
     * For count<=31, floor(span/min)+1<=32-count iff
     * span<(32-count)*min. Product<=32*65535 cannot overflow uint32_t.
     * Keep the full FIFO, freshness, transaction and both bucket divisions.
     */
    if ((uint32_t)r->count >= WS_FIFO_FRAMES ||
        span >= (WS_FIFO_FRAMES - (uint32_t)r->count) * s->profile.minimum_period_ms)
        return fail(s, d);
    ...
    for (uint8_t i = 0; i < r->count; i++) {
        ...
        if ((uint32_t)(t.latest - t.earliest) > s->profile.timestamp_uncertainty_ms ||
            ...
            first >= 0x80000000u ||
            first / WS_PERIOD_MS != last / WS_PERIOD_MS) return fail(s, d);
        if (next.have_frame &&

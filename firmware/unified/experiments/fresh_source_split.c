/* UNADOPTED source-splitting experiment; accepted fresh_source.c is unchanged.
 * Private helpers are explicitly outlined to expose whole-module code/stack
 * tradeoffs. This is not production placement, a size win, or stack ownership.
 */
...
static __attribute__((noinline)) bool preflight(const ws_source *s, const ws_receipt *r, uint32_t now) {
...
static __attribute__((noinline)) bool frame(const ws_source *s, const ws_receipt *r,

...
wt_result wr_offer(wr_runtime *r, uint32_t session, uint32_t acquisition,
    ...
    if (result != WT_OK) {
        ...
        for (uint8_t i = 0; i < count; i++)
            r->queued_at[(slot + i) % WT_CAPACITY] = (uint16_t)oldest_at;
...
bool wr_next(wr_runtime *r, uint32_t session, wt_sample *sample, uint32_t now) {
    ...
    if (r->tap.count && (uint16_t)(now - r->queued_at[r->tap.head]) >= WR_MAX_SAMPLE_AGE_MS) {
        wm_stream_failed(&r->mode, now);
        discard_if_not_gesture(r);
        return false;
    }
    r->pending_at = now - (uint16_t)(now - r->queued_at[r->tap.head]);

...
/* SCRATCH ONLY: share the identical guarded resume admission sequence. */
static __attribute__((noinline)) bool resume_current(wa_adapter *a, uint32_t token, uint32_t now) {
    if (!action_matches(a, token, a->runtime.mode.session, WM_RESUME_HEALTH)) return false;
    wa_tick(a, now);
    return a->runtime.mode.token == token && a->runtime.mode.action == WM_RESUME_HEALTH;
}

bool wa_begin_resume(wa_adapter *a, uint32_t token, uint32_t revision, uint32_t now) {
    if (!resume_current(a, token, now)) return false;

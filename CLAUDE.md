...
| App image | file `0x450` = abs `0x00826400`; `.data` file `0x20ae8` -> RAM `0x207c00`; bss from `0x208994` |
| Accelerometer | STK8321, I2C0 slave `0x1f`, INT on P2_6 |
| Optical | VCare VC30F ("core30fx_v0.23"), I2C0 slave `0x33`, INT on P4_1, chip ID `0x25`/`0x27` |
| Battery | 17 mAh |
...
LEDs and holding the BLE link. With the emitters off (host workaround, see
"LED"), a 10-minute run measured **0.20 %/min** (92% -> 90%) against 0.30 with
them on; a runtime figure needs a 3 h matched run. Phase A (1 h session) has
ample margin either way.

**A stream can pass rate and loss while carrying no motion.** The
accelerometer driver idles the chip after a quiet spell and the producer
repeats a cached sample (see "LED"). `analyze` now reports frozen runs; read
them next to rate and loss. The two ambient hours were 28-39% frozen.

---

## LED

**Solved 2026-09-22.** The emitters were never inseparable from the
accelerometer; the stream's stop command was wrong, and the raw producer never
needed the optical sensor. Everything below was read from the disassembly of
`rt02cr-25hz.bin` and checked by two independent review passes (nine verifier
agents on the mechanics, six on the v2 patch); the hardware runs were done with
the ring in hand. Long form: `docs/FIRMWARE_RESEARCH.md`; chronology:
`docs/LED_FIX.md`; tools and captures: `firmware/research/2026-09-22/`.

**How the disassembly became readable.** The image is Realtek RTL8762E
(Cortex-M0+), not BlueX: 79 of 79 `bl` targets below `0x100000` match the
RTL8762E ROM symbol table (`os_timer_create 0x13635`, `memcpy 0x3f849`, ...).
The load base is in the entry vector: file `0x450` is `ldr r0,[pc]; bx r0;
.word 0x00826665`, so file `0x450` = abs `0x00826400`; at that base 56 of 80
odd flash literals land on `push {..,lr}` (next-best base: 13). The earlier
"could not solve the load base" was a search that never read the vector.
`firmware/research/2026-09-22/fwmap.py` builds an annotated function map
(`func`, `xref`, `grep`, `calls`); function names are `sub_<file offset>`.

**The A1 handler is `sub_020bc`**, reached from the BLE dispatcher
`sub_0564a` through a 16-byte ring queue and the task dispatcher `sub_06334`.
Byte 1 selects a sub-command, and each start has a matching stop that clears
one bit in the sensor task's u16 mask at RAM `0x20c018`:

| `A1` | does | sensor bit |
|---|---|---|
| `01` / `06` / `07` | PPG raw modes, 1 Hz or 32 ms producer | enable `0x40` |
| `02` / `08` | stop those | clear `0x40` only |
| **`04`** | disable all (`0xffff`), enable **`0x800`**, mode 4, 32 ms producer timer (the `#4` at `0x2248`) | enable `0x800` |
| **`05`** | disable `0x800`, mode 0, stop + delete the producer timer | clear `0x800` |
| `03` | one synchronous report | -- |

Every `A1` command is refused with `A1 FF` while charging (`sub_02d4e`,
bit 0 of `0x209ce4`).

**Why the LEDs lit and why they stuck.** Enable is handled by `sub_0f68c`:
with the mask at zero and bits `0x40` or `0x800`, it configures the VC30F in
mode 7 (`sub_0fec4`: slot 0 and slot 1 emitters at current `0x7f`, plus slot 2
at `0xb`) and starts it (`sub_0eff8` -> `sub_10c86` -> `sub_10016` ->
register `0x7b <- 0x5a`). That is the flicker. `A1 02` clears `0x40`, not
`0x800`, so after every `A1 04` session the mask still held `0x800`, the chip
kept running, and only a power cycle cleared it. **`A1 05` is the stop**:
`sub_0f740(0x800)` takes the mask to zero and calls `sub_10d12`, which writes
`0x7b <- 0xa5, 0x00`. On hardware the LEDs went dark after `A1 05` with no
charger tap. The charger tap worked because the charger handler sends
disable-all (`sub_033ac` -> `sub_0e3b6` -> `sub_0dc82(0xffff)`), not because
of anything in the A1 path. `protocol.STOP_RAW_SENSOR_PACKETS` now sends
`A1 05` then `A1 02`, and `capture.stream` sends them on every stop.

**The accelerometer never needed the optical sensor.** The 32 ms producer
`sub_01dfe` builds the `A1 03` frame from `sub_0cbda`, which reads the
newest sample from the accelerometer driver's ring buffer at `0x20bdc4+0xc`
(filled by the STK8321 interrupt path: ISR `sub_0bb12` on P2_6 -> `sub_0ca60`
-> FIFO drain `sub_0c228` over I2C, slave `0x1f`), draining the FIFO itself
when flag `0x20bdc5` is set. Nothing in that path touches the VC30F (I2C
slave `0x33`, wrappers `sub_0ecaa`/`sub_0ece2`, INT on P4_1).

**But the accelerometer has its own sleep, and that is the real trap.** After
a quiet spell (`sub_0cd08` counts 125 quiet samples via `sub_1d91c`) the
driver posts an idle request (`0x20bdca`), `sub_0ca60` reconfigures the
STK8321 into low-power any-motion mode (`sub_0c888`: registers `0x0F..0x11` go
from `05 0F 74` to `05 0A 7C`, interrupt map `0x1A` -> 0) and clears
`0x20bdc5`; from then on `sub_0cbda` returns the same cached sample every
tick until an any-motion interrupt re-initialises the chip (SWRST + 50 ms).
**The stream keeps its 25 Hz and its 0% loss while carrying no motion.** The
optical sensor's sample processing (`sub_0ed20`) also reads the FIFO, which
is why the problem was masked while the LEDs were on. Hardware confirmed it:
with the VC30F stopped mid-stream, packets kept coming at 25 Hz with one XYZ
value for 57 s (`docs/LED_FIX.md` evidence 3-4). Two consequences:

- The archived captures were audited (`python -m probe.freshness`; the
  metric is in `analyze.StreamStats` as `frozen_runs` / `frozen_s` /
  `longest_frozen_s` / `distinct_fraction`, and `probe.report` prints it).
  **The two ambient hours were frozen for 28% and 39% of their length**
  (`negative_20260915_021616`: 149 runs, longest 59 s;
  `negative_20260915_224235`: 114 runs, longest 84 s). A frozen stretch cannot
  produce a false positive, so the ambient false-positive rates in this file
  are bounded on 0.6-0.7 of the wear time they claim. Prompted gesture
  sessions were under 3% frozen; the 10-minute gate capture 1.1%; the desk
  captures from the flashing days 75-100% (ring not worn). Re-measure the
  ambient rate on captures that pass the freshness check.
- The host can hold the chip awake: `3B 02 01 03` (volatile motion-action
  mode 3; `sub_05c44` -> `0x20bfc0`, wake via `sub_0c4fc`, idle inhibited by
  `sub_0d43e` while connected). Its dispatch is a no-op in this firmware;
  mode 1 is not interchangeable (it can emit host input events). Disable with
  `3B 02 01 00`; the setter forces the sensitivity byte, so the diagnostic
  requires the pre-test state `00 01 00` and restores it.

**LED-off tracking on the flashed firmware, validated on hardware.** After
`A1 04`, send `3B 02 01 03` (wake/hold) and then `CE 02 33 7B 01 00 ...`
(the vendor's raw I2C write: VC30F register `0x7b <- 0x00`, STOP). Result:
25.00 Hz, every frame distinct, LEDs dark for the whole 120 s interval with
the wearer moving, user-observed; and a 10-minute run at **0.20 %/min**
against 0.30 %/min with the emitters on. Caveats: `CE 02` writes any byte to
any I2C register and belongs only in `probe.ledcheck` / `probe.batterycheck`,
never the normal capture path; background optical starts stay possible on this
image (below); the 10-minute figure is not a runtime measurement.

**What can still light the emitters on this image.** The minute tick
`sub_01202` runs the health schedule while the time-set flag `0x208c4a` is 1:
`0x16` HR at its interval (bit 0 of `0x208ab1`), `0x2c` SpO2 at minute 32
(bit 1), `0x36`, `0x38`, `0x3a` (bits 2-4). `16 02 02 3c` clears bit 0 only;
`2c 02 02` bit 1; bits 2-4 read zero on this ring except `0x3a`, which was
never read back. While a schedule measurement runs with `0x800` in the mask,
`sub_0f740` restarts the VC30F in mode 0 when it ends and leaves it running --
the "LEDs came on ~90 s in and stayed" observation. `69 01 04` is a START of
realtime heart rate (bit `0x1`), not a stop; the stop is `69 06 04`. With any
extra bit in the mask, `A1 05` no longer reaches `sub_10d12` until that
feature clears its own bit. Boot, charger, find-ring (`0x50`) and low-battery
indicator patterns start the chip through `sub_03b8c`/`sub_03c16` ->
`sub_110ae`, gated on mask == 0.

**Firmware candidates -- nothing flashed.**

- `python -m probe.build --no-optical` (one halfword, `0xf710`: `51 48` ->
  `e5 e7`, `b` to the enable handler's `pop`, located by signature) skips the
  mode-7 start and leaves the mask clear. **Superseded, do not flash**: it
  does not hold the accelerometer awake, so the stream freezes whenever the
  wearer is still, and it leaves every idle-time optical start live. Kept
  because the site and signature are the documented anchor.
- `whip/fwoptical.py` -> `firmware/rt02cr-25hz-optical-off-v2-experimental.bin`
  (sha256 `0d18a0fa...`, 81 payload bytes): `sub_0f68c` becomes `bx lr`
  (no optical enable at all); the VC30F RUN value at `0x1231a` becomes STOP
  (indicators and recoveries dark too); `A1 04` stores mode 4 and requests the
  accelerometer wake (`sub_0cb06`); an idle guard in the reclaimed enable
  body (`0xf690`, called from `0xcad2`) refuses the STK idle request while
  mode is 4 and BLE state `0x209e09` is 2; a disconnect wrapper (`0xf6b4`,
  hooked at `0x691e`) clears mode 4 and stops the producer timer, since the
  base image keeps the timer and the DLPS veto (`sub_0a54e`) alive after a
  drop. Identity after a flash cannot come from the version string (identical
  to `#4`); `probe.validate_optical_off` reads 22 code sites over `CD 01`.
  Bench protocol and the independent review of the six sites: see the
  FIRMWARE_RESEARCH resume checklist and the section appended below when the
  review lands. Roll back with `firmware/rt02cr-25hz.bin`.

**Battery, what remains once the emitters are off.** Raw mode 4 vetoes deep
sleep (`sub_0a54e` -> `sub_01df6`, mode byte == 4), the 32 ms timer and
notify run at the central's connection interval (the ring's connection-
parameter request is stubbed at `0x9614`), and the STK8321 is polled every
tick. On the base image a disconnect leaves mode 4 set, so the producer keeps
firing and sleep stays vetoed until the host reconnects and sends `A1 05` --
send the stop sequence at the start of every connection as well as the end.

**Corrections to what this file said before.** `A1 04` "means power the whole
sensor front end" -- no, it enables one bit and the enable handler chose the
optical mode. "No `A1` parameter streams motion alone" -- `A1 04` with the
sensor stopped does. "The realtime stop commands do nothing" -- `69 01 04`
was a start. "Following the timer callback needs the load base, which would
not resolve" -- it is the first word of the image. "~5100 call sites, past
bisection" -- the path is `sub_0564a` -> `sub_06334` -> `sub_020bc` ->
`sub_0dc9c` -> `sub_0dcd0` -> `sub_0f68c`, five hops.
...
- **LEDs off is done at the protocol level; the firmware candidate is
  unflashed.** Next: bench the v2 image per the review's protocol (fresh XYZ
  still/moving for 5+ min, three start/stop cycles, disconnect mid-stream,
  charger insert, 3 h battery run), or keep the host workaround. Either way,
  re-measure the ambient false-positive rate on captures that pass
  `probe.freshness`, because the existing ambient hours were 28-39% frozen.
